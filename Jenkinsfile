pipeline{
    ...
    parameters {
        choice(name: 'select_environment', choices: ['apply', 'destroy'], description: 'choices one option for create/destroy infra')
    ...
    stages {
        ...
        stage ('terraform apply') {
            when { 
                  expression { params.select_environment == 'apply' }
            }
            steps {
        ...
        stage ('terraform destroy') {
            when { 
                  expression { params.select_environment == 'destroy' }
            }
            steps {
        ...
        stage("Install_MySQL_Create-Table") {
              when { 
                  expression { params.table == 'create' && params.select_environment == 'apply' }
              }
              steps {
                 // sh 'ansible-playbook -i ${ANSIBLE_INVENTORY} ${ANSIBLE_PLAY_CR_PATH} -u ubuntu'
        ...
        stage("Delete-Table") {
            when { 
                  expression { params.table == 'delete' && params.select_environment == 'apply' }
            }
            steps {
              //  sh 'ansible-playbook -i ${ANSIBLE_INVENTORY} ${ANSIBLE_PLAY_CR_PATH} -u ubuntu '
                ansiblePlaybook credentialsId: '2a4dea98-3c79-4475-908f-9f764acd5762', disableHostKeyChecking: true, installation: 'ansible',
